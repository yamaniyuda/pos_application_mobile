...
import 'package:pos_application_mobile/domain/use_cases/cloth_price_type/delete_data_use_case.dart';
import 'dart:async';

import 'package:pos_application_mobile/domain/use_cases/cloth_price_type/fetch_data_use_case.dart';
import 'package:pos_application_mobile/domain/use_cases/cloth_price_type/store_data_use_case.dart';
import 'package:pos_application_mobile/domain/use_cases/cloth_price_type/update_data_use_case.dart';
...
class ClothPriceTypeController extends GetxController {
  /// Get depedency from auth service
  final AuthService authService = Get.find<AuthService>();

  /// Fetch data use case instance for  data.
  final FetchDataUseCase fetchDataUseCase = Get.put(FetchDataUseCase());

  /// Store data use case instance for  data.
  final StoreDataUseCase storeDataUseCase = Get.put(StoreDataUseCase());
  ...
  final RxList<ClothPriceTypeEntity> _data = <ClothPriceTypeEntity>[].obs;
  List<ClothPriceTypeEntity> get data => _data.value;
  ...
  Future<void> fetchData({bool refresh = false, Map<String, dynamic>? queryParameters}) async {
    ...
    List<ClothPriceTypeEntity> data = await fetchDataUseCase.call(buildQueryParameters);
  ...
  /// 
  /// Parameters:
  /// - `paylaod`: A [ClothPriceTypePayload] entity while will store to remote data source.
  /// 
  /// Usage example:
  /// ```dart
  /// storeData(payload(/*...*/))
  /// ```
  void storeData(ClothPriceTypePayload paylaod) async {
    try {
  ...
  void deleteData(String id) async {
    try {
      ...
      List<ClothPriceTypeEntity> currentData = _data.value;
      _data.value = [];
  ...
  /// 
  /// Parameters:
  /// - `payload`: A [ClothPriceTypePayload] payload to update data.
  /// 
  /// 
  /// Usage example:
  /// ```dart
  /// updateData(ClothPriceTypePayload(/*...*/))
  /// ```
  void updateData(ClothPriceTypePayload paylaod) async {
    try {
      PAMAlertWidget.showLoadingAlert(Get.context!);
