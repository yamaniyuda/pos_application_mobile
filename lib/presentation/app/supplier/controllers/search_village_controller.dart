...
import 'package:pos_application_mobile/domain/use_cases/village/fetch_data_use_case.dart';

/// Controller responsible for managing Customer-related operations.
class SearchVillageController extends GetxController {

  final Rx<String> _districtId = "".obs;
  String get districtId => _districtId.value;
  set setdistrictId(String districtId) => _districtId.value = districtId;

  /// Fetch data use case instance for Customer data.
  final FetchDataUseCase fetchDataUseCase = Get.put(FetchDataUseCase(), tag: 'village');

  /// List of fetched district data.
  final RxList<VillageEntity> _villageDatas = <VillageEntity>[].obs;
  List<VillageEntity> get villageDatas => _villageDatas.value;
  ...
  Future<void> fetchDataCustomer({bool refresh = false, Map<String, dynamic>? queryParameters}) async {
    ...
    List<VillageEntity> data = await fetchDataUseCase.call(buildQueryParameters);

    // Update state with fetched data
    _villageDatas.value.addAll(data);
