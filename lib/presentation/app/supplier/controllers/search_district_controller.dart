...
import 'package:pos_application_mobile/domain/use_cases/district/fetch_data_use_case.dart';

/// Controller responsible for managing Customer-related operations.
class SearchDistrictController extends GetxController {

  final Rx<String> _regencyId = "".obs;
  String get regencyId => _regencyId.value;
  set setDistrictId(String districtId) => _regencyId.value = districtId;

  /// Fetch data use case instance for district data.
  final FetchDataUseCase fetchDataUseCase = Get.find<FetchDataUseCase>(tag: 'district');

  /// List of fetched district data.
  final RxList<DistrictEntity> _districtDatas = <DistrictEntity>[].obs;
  List<DistrictEntity> get districtDatas => _districtDatas.value;
  ...
  Future<void> fetchDataCustomer({bool refresh = false, Map<String, dynamic>? queryParameters}) async {
    ...
    List<DistrictEntity> data = await fetchDataUseCase.call(buildQueryParameters);

    // Update state with fetched data
    _districtDatas.value.addAll(data);
